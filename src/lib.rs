...
pub fn create_app(state: AppState) -> Router {
    Router::new()
        ...
        // Admin-only: the audit trail records the Actor behind every change, across
        // every Producer. `admin_auth` validates sessions only, so API tokens cannot
        // read it — deliberate, and safe because this route is outside `api.yaml`.
        .route("/api/audit/timeline", get(api::audit_timeline).layer(from_fn_with_state(state.clone(), admin_auth)))
        ...
        .route("/admin/observability/audit-logs", get(admin::get_observability_audit_logs).layer(from_fn_with_state(state.clone(), admin_auth)))
        .route("/admin/observability/audit-logs/export", get(admin::export_audit_logs_csv).layer(from_fn_with_state(state.clone(), admin_auth)))
