...
pub fn split_proto(content: &str) -> Result<Vec<ProtoSpec>, String> {
        ...
        if content[i..].starts_with("service ") {
            ...
            if let Some(brace_start_rel) = content[i..].find('{') {
                ...
                while brace_count > 0 && j < bytes.len() {
                    if bytes[j] == b'{' { brace_count += 1; }
                    else if bytes[j] == b'}' { brace_count -= 1; }
                    j += 1;
                }
    ...
    for line in content.lines() {
        ...
        if in_service {
            ...
            if trimmed.contains('}') { brace_count -= 1; }
            if brace_count == 0 { in_service = false; }
            continue;
        }
...
"#;
        ...
        let get_user = result.iter().find(|s| s.service == "UserService" && s.method == "GetUser").unwrap();
        ...
        let check = result.iter().find(|s| s.service == "HealthService" && s.method == "Check").unwrap();
