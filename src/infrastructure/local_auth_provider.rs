...
mod tests {
    ...
    async fn seed_users(repo: &SqliteSpecRepository) {
        ...
        let hash = argon2.hash_password("password123".as_bytes(), &salt).unwrap().to_string();
        
        repo.create_user("approved_user", &hash, false, true).await.unwrap();
        repo.create_user("unapproved_user", &hash, false, false).await.unwrap();
    }
    ...
    async fn test_authenticate_success() {
        ...
        assert!(result.is_ok(), "Result failed with: {:?}", result.err().unwrap());
    ...
    async fn test_authenticate_invalid_password() {
        ...
        let result = provider.authenticate("approved_user", "wrongpassword").await;
    ...
    async fn test_authenticate_unapproved_user() {
        ...
        let result = provider.authenticate("unapproved_user", "password123").await;
        assert!(matches!(result, Err(AuthProviderError::InvalidCredentials)));
    }
    ...
    async fn test_authenticate_hash_parse_error() {
        let repo = setup_db().await;
        repo.create_user("badhash", "not-a-valid-argon2-hash", false, true).await.unwrap();
        let provider = LocalAuthProvider::new(repo);
    ...
    async fn test_connection_db_failure() {
        ...
        let result = provider.test_connection().await;
        assert!(matches!(result, Err(AuthProviderError::ConnectionFailed(_))));
    }
