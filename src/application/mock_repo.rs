...
use crate::domain::ports::{RecordDependencyParams, RepositoryError, SpecRepository, EndpointMap};
...
impl SpecRepository for MockRepo {
    ...
    async fn record_dependency(&self, _params: RecordDependencyParams<'_>) -> Result<(), RepositoryError> {
        Ok(())
    }

    async fn record_dependencies_bulk(&self, _params: Vec<RecordDependencyParams<'_>>) -> Result<(), RepositoryError> {
        Ok(())
    }

    async fn find_endpoint(&self, _service_id: i64, _branch_name: &str, _api_type: ApiType, _path: &str, _method: &str) -> Result<Option<(i64, String)>, RepositoryError> {
        Ok(None)
    }

    async fn find_endpoints_bulk(&self, _service_id: i64, _branch_name: &str, _api_type: ApiType, _endpoints: &[(String, String)]) -> Result<EndpointMap, RepositoryError> {
        Ok(HashMap::new())
    }
    ...
    async fn update_endpoint(&self, _branch_id: i64, _api_type: ApiType, _path: &str, _method: &str, _yaml: &str) -> Result<(), RepositoryError> {
        Ok(())
    }
    ...
    async fn delete_branch(&self, _service_name: &str, _branch_name: &str) -> Result<bool, RepositoryError> {
        Ok(true)
    }
    ...
    async fn get_fallback_branch(&self, service_name: &str) -> Result<Option<String>, RepositoryError> {
        Ok(self.fallback_branches.lock().unwrap().get(service_name).cloned())
    }
    ...
    async fn create_api_token(&self, _id: &str, _user_id: i64, _name: &str, _hash: &str, _created: &str, _expires: &str) -> Result<(), RepositoryError> {
        Ok(())
    }
    ...
    async fn delete_api_token(&self, _token_id: &str, _user_id: i64) -> Result<bool, RepositoryError> {
        Ok(true)
    }
    ...
    async fn get_endpoint_id(&self, _branch_id: i64, _api_type: ApiType, _path: &str, _method: &str) -> Result<Option<i64>, RepositoryError> {
        Ok(None)
    }

    async fn insert_endpoint_version(&self, _endpoint_id: i64, _version: i32, _yaml: &str, _diff: Option<&str>, _created: &str) -> Result<(), RepositoryError> {
        Ok(())
    }
    ...
    async fn get_endpoint_versions(&self, _endpoint_id: i64) -> Result<Vec<EndpointVersion>, RepositoryError> {
        Ok(Vec::new())
    }
        ...
        for change in changes {
            match change {
                ...
                SpecChange::Update { api_type, path, normalized_path, method, yaml_content } => {
                    if let Some(ep) = list.iter_mut().find(|e| e.api_type == api_type && e.normalized_path == normalized_path && e.method == method) {
                        ep.path = path;
                ...
                SpecChange::Delete { api_type, path, method, soft_delete } => {
                    if soft_delete || is_protected {
                        ...
                        list.retain(|e| !(e.api_type == api_type && e.path == path && e.method == method));
                    }
                }
            }
        }
    ...
    async fn add_service_tags(&self, service_id: i64, tags: &[String]) -> Result<(), RepositoryError> {
        ...
        st.entry(service_id).or_default().extend(tags.iter().cloned());
        Ok(())
    }
    ...
    async fn get_shared_contract(&self, _branch: &str, _api_type: ApiType, _path: &str, _method: &str) -> Result<Option<SharedContract>, RepositoryError> {
        Ok(None)
    }

    async fn upsert_shared_contract(&self, _contract: SharedContract) -> Result<(), RepositoryError> {
        Ok(())
    }
}
