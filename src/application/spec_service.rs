...
async fn provide_spec_inner(
        ...
        if !existing.is_empty() {
            ...
            if !existing_yamls.is_empty() {
                ...
                if let Err(reason) = openapi::check_backward_compatibility(&old_full_yaml, content) {
                    tracing::warn!("Rejected update for service '{}' branch '{}': breaking changes: {}", servicename, branch, reason);
                    return Err(AppError::Conflict(format!(
    ...
    if dry_run {
        return Ok(ProvideResponse { version: current_version, content_hash, changes: ProvideChanges { inserts, updates, deletes } });
    }
    ...
    Ok(ProvideResponse { version: new_version, content_hash, changes: ProvideChanges { inserts, updates, deletes } })
}

pub async fn get_endpoint_yaml(repo: &impl SpecRepository, servicename: &str, branch: &str, api_type: ApiType, path: &str, method: &str) -> Result<String, AppError> {
    ...
    endpoint.map(|(_, yaml)| yaml).ok_or_else(|| AppError::NotFound(format!("Endpoint not found: {} {}", method_to_use, path)))
}

pub async fn list_service_endpoints(repo: &impl SpecRepository, servicename: &str, branch: &str) -> Result<Vec<EndpointRecord>, AppError> {
    let service_id = repo.ensure_service(servicename).await?;
    let branch_id = repo.ensure_branch(service_id, branch).await?;
    Ok(repo.get_endpoints_for_branch(branch_id).await?)
}

pub async fn get_endpoint_version_history(repo: &impl SpecRepository, servicename: &str, branch: &str, api_type: ApiType, path: &str, method: &str) -> Result<Vec<EndpointVersion>, AppError> {
    let service_id = repo.ensure_service(servicename).await?;
    let branch_id = repo.ensure_branch(service_id, branch).await?;
    let method_to_use = match api_type {
        ApiType::OpenApi | ApiType::AsyncApi => method.to_uppercase(),
...
pub async fn require_endpoint(repo: &impl SpecRepository, notifier: Option<tokio::sync::broadcast::Receiver<()>>, params: RequireEndpointParams<'_>) -> Result<String, AppError> {
    require_endpoint_inner(repo, notifier, params, false).await
}

pub async fn require_endpoint_dry_run(repo: &impl SpecRepository, notifier: Option<tokio::sync::broadcast::Receiver<()>>, params: RequireEndpointParams<'_>) -> Result<String, AppError> {
    require_endpoint_inner(repo, notifier, params, true).await
}

async fn require_endpoint_inner(repo: &impl SpecRepository, mut notifier: Option<tokio::sync::broadcast::Receiver<()>>, params: RequireEndpointParams<'_>, dry_run: bool) -> Result<String, AppError> {
    let client_id = if dry_run { 0 } else { repo.ensure_client(params.clientname).await? };
    let service_id = if dry_run {
        repo.find_service(params.servicename).await?.ok_or_else(|| AppError::NotFound("Service not found".to_string()))?
    } else {
    ...
    loop {
        ...
        if let Some((id, yaml)) = endpoint {
            if !dry_run {
                repo.record_dependency(RecordDependencyParams { client_id, endpoint_id: Some(id), api_type: params.api_type, service_id, branch_name: params.branch, path: params.path, method: &method_to_use }).await?;
            }
            return Ok(yaml);
        }
        ...
        if let Some(dl) = deadline {
            ...
            if now < dl {
                ...
                match notifier.as_mut() {
                    Some(rx) => { tokio::select! { _ = tokio::time::sleep(timeout) => {}, _ = rx.recv() => {}, } }
                    None => { tokio::time::sleep(poll_interval.min(timeout)).await; }
                }
                continue;
            }
        }
        ...
        if !dry_run {
            repo.record_dependency(RecordDependencyParams { client_id, endpoint_id: None, api_type: params.api_type, service_id, branch_name: params.branch, path: params.path, method: &method_to_use }).await?;
        }
        return Err(AppError::NotFound(format!("Endpoint not found: {} {}", method_to_use, params.path)));
    }
}

pub async fn require_bundle(repo: &impl SpecRepository, notifier: Option<tokio::sync::broadcast::Receiver<()>>, params: RequireBundleParams<'_>) -> Result<String, AppError> {
    require_bundle_inner(repo, notifier, params, false).await
}

pub async fn require_bundle_dry_run(repo: &impl SpecRepository, notifier: Option<tokio::sync::broadcast::Receiver<()>>, params: RequireBundleParams<'_>) -> Result<String, AppError> {
    require_bundle_inner(repo, notifier, params, true).await
}

async fn require_bundle_inner(repo: &impl SpecRepository, mut notifier: Option<tokio::sync::broadcast::Receiver<()>>, params: RequireBundleParams<'_>, dry_run: bool) -> Result<String, AppError> {
    if params.endpoints.is_empty() { return Err(AppError::BadRequest("No endpoints requested".to_string())); }
    let client_id = if dry_run { 0 } else { repo.ensure_client(params.clientname).await? };
    let service_id = if dry_run {
        repo.find_service(params.servicename).await?.ok_or_else(|| AppError::NotFound("Service not found".to_string()))?
    } else {
    ...
    loop {
        let normalized_endpoints: Vec<(String, String)> = params.endpoints.iter().map(|(p, m)| {
            (p.clone(), match params.api_type { ApiType::OpenApi | ApiType::AsyncApi => m.to_uppercase(), ApiType::Proto => m.to_string() })
        }).collect();

        let found_endpoints = find_endpoints_bulk_with_fallback(repo, service_id, params.servicename, params.branch, params.api_type, &normalized_endpoints).await?;

        if found_endpoints.len() == params.endpoints.len() {
            let mut yamls = Vec::new();
            let mut bulk_params = Vec::new();
            for (path, method) in params.endpoints {
                let m_use = match params.api_type { ApiType::OpenApi | ApiType::AsyncApi => method.to_uppercase(), ApiType::Proto => method.to_string() };
                let (id, yaml) = found_endpoints.get(&(path.clone(), m_use)).unwrap();
                yamls.push(yaml.clone());
                bulk_params.push(RecordDependencyParams { client_id, endpoint_id: Some(*id), api_type: params.api_type, service_id, branch_name: params.branch, path, method });
            }
            if !dry_run { repo.record_dependencies_bulk(bulk_params).await?; }
...
async fn find_endpoint_with_fallback(repo: &impl SpecRepository, service_id: i64, servicename: &str, branch: &str, api_type: ApiType, path: &str, method_to_use: &str) -> Result<Option<(i64, String)>, RepositoryError> {
    let endpoint = repo.find_endpoint(service_id, branch, api_type, path, method_to_use).await?;
    if endpoint.is_some() || repo.is_branch_protected(branch).await? { return Ok(endpoint); }
    ...
    for pb in &protected {
        if pb != branch
            && let Some(ep) = repo.find_endpoint(service_id, pb, api_type, path, method_to_use).await? {
            return Ok(Some(ep));
        }
    }
    Ok(None)
...
async fn find_endpoints_bulk_with_fallback(repo: &impl SpecRepository, service_id: i64, servicename: &str, branch: &str, api_type: ApiType, endpoints: &[(String, String)]) -> Result<crate::domain::ports::EndpointMap, RepositoryError> {
    let mut results = repo.find_endpoints_bulk(service_id, branch, api_type, endpoints).await?;
    let mut missing: Vec<(String, String)> = endpoints.iter().filter(|e| !results.contains_key(&(e.0.clone(), e.1.clone()))).cloned().collect();
    if missing.is_empty() || repo.is_branch_protected(branch).await? { return Ok(results); }
    ...
    if let Some(sfb) = repo.get_fallback_branch(servicename).await.ok().flatten().filter(|b| b != branch) {
        ...
        for (key, val) in fallback_results { results.insert(key.clone(), val); missing.retain(|m| m.0 != key.0 || m.1 != key.1); }
    }
...
pub fn check_compatibility(api_type: ApiType, old_yaml: &str, new_yaml: &str) -> Result<(), String> {
    match api_type {
        ApiType::OpenApi => {
            openapi::check_backward_compatibility(old_yaml, new_yaml)
        }
        _ => Ok(()),
    }
}
