...
- **Event-driven Long-polling**: replaced the 500ms busy-wait loop in `/require` and `/require-bundle` with a `tokio::sync::broadcast` mechanism. Waiting clients now wake up immediately when a new spec is provided, reducing latency and server load.
