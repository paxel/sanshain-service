...
async fn app() -> axum::Router {
    ...
    create_app(mk_state(repo))
}

#[tokio::test]
async fn metrics_endpoint_has_text_content_type() {
    let app = app().await;
    let res = app
        .oneshot(
            Request::builder()
...
#[tokio::test]
async fn license_endpoint_returns_text() {
    let app = app().await;
    let res = app
        .oneshot(
            Request::builder()
    ...
    assert!(text.to_lowercase().contains("license"));
}
