...
async fn execute_sqlite_migration(pool: &sqlx::SqlitePool, sql: &str) {
...
async fn execute_postgres_statements(pool: &sqlx::PgPool, sql: &str) {
    for statement in sql
        .split(';')
        .map(str::trim)
        .filter(|statement| !statement.is_empty())
...
#[tokio::test]
async fn test_sqlite_api_type_migration_handles_existing_dependencies() {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
    ...
    sqlx::query("CREATE TABLE branches (id INTEGER PRIMARY KEY AUTOINCREMENT, service_id INTEGER NOT NULL, name TEXT NOT NULL, UNIQUE(service_id, name), FOREIGN KEY(service_id) REFERENCES services(id))")
        ...
        .await
        .unwrap();
    sqlx::query("CREATE TABLE endpoints (id INTEGER PRIMARY KEY AUTOINCREMENT, branch_id INTEGER NOT NULL, path TEXT NOT NULL, normalized_path TEXT NOT NULL, method TEXT NOT NULL, yaml_content TEXT NOT NULL, deleted BOOLEAN NOT NULL DEFAULT FALSE, UNIQUE(branch_id, path, method), FOREIGN KEY(branch_id) REFERENCES branches(id))")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query(
    ...
    sqlx::query("INSERT INTO services (id, name) VALUES (1, 'orders')")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO branches (id, service_id, name) VALUES (1, 1, 'main')")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO endpoints (id, branch_id, path, normalized_path, method, yaml_content, deleted) VALUES (1, 1, '/orders/{id}', '/orders/{}', 'GET', 'openapi: 3.0.0', FALSE)")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO clients (id, name) VALUES (1, 'checkout')")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO dependencies (id, client_id, endpoint_id, requested_service_id, requested_branch_name, requested_path, requested_normalized_path, requested_method) VALUES (1, 1, 1, 1, 'main', '/orders/123', '/orders/{}', 'GET')")
        .execute(&pool)
        .await
        .unwrap();
    ...
    execute_sqlite_migration(
        &pool,
        include_str!("../src/infrastructure/migrations/sqlite/20240316000000_api_type.sql"),
    )
    .await;
    ...
    assert_eq!(endpoint_api_type, "openapi");
    assert_eq!(dependency_api_type, "openapi");
}

#[tokio::test]
async fn test_sqlite_null_endpoint_unique_migration_deduplicates_unresolved_dependencies() {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
    ...
    sqlx::query("CREATE TABLE dependencies (id INTEGER PRIMARY KEY AUTOINCREMENT, client_id INTEGER NOT NULL, endpoint_id INTEGER, api_type TEXT NOT NULL DEFAULT 'openapi', requested_service_id INTEGER NOT NULL, requested_branch_name TEXT NOT NULL, requested_path TEXT NOT NULL, requested_normalized_path TEXT NOT NULL, requested_method TEXT NOT NULL, last_seen_at TEXT NOT NULL DEFAULT '1970-01-01T00:00:00')")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO dependencies (id, client_id, endpoint_id, api_type, requested_service_id, requested_branch_name, requested_path, requested_normalized_path, requested_method, last_seen_at) VALUES (1, 7, NULL, 'openapi', 9, 'main', '/orders/{id}', '/orders/{}', 'GET', '2026-04-28T06:00:00')")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO dependencies (id, client_id, endpoint_id, api_type, requested_service_id, requested_branch_name, requested_path, requested_normalized_path, requested_method, last_seen_at) VALUES (2, 7, NULL, 'openapi', 9, 'main', '/orders/{id}', '/orders/{}', 'GET', '2026-04-28T05:00:00')")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO dependencies (id, client_id, endpoint_id, api_type, requested_service_id, requested_branch_name, requested_path, requested_normalized_path, requested_method, last_seen_at) VALUES (3, 7, NULL, 'asyncapi', 9, 'main', '/orders/{id}', '/orders/{}', 'GET', '2026-04-28T04:00:00')")
        .execute(&pool)
        .await
        .unwrap();
    ...
    let remaining_ids: Vec<i64> = sqlx::query_scalar("SELECT id FROM dependencies ORDER BY id")
        .fetch_all(&pool)
        .await
        .unwrap();
    assert_eq!(remaining_ids, vec![2, 3]);

    let duplicate_insert = sqlx::query("INSERT INTO dependencies (client_id, endpoint_id, api_type, requested_service_id, requested_branch_name, requested_path, requested_normalized_path, requested_method, last_seen_at) VALUES (7, NULL, 'openapi', 9, 'main', '/orders/{id}', '/orders/{}', 'GET', '2026-04-28T07:00:00')")
        .execute(&pool)
        .await;
...
#[tokio::test]
async fn test_postgres_api_type_migration_handles_default_unique_constraint_names_when_available() {
    ...
    execute_postgres_statements(
        &pool,
        include_str!("../src/infrastructure/migrations/postgres/20240308000000_initial_schema.sql"),
    )
    .await;
    sqlx::query("INSERT INTO services (id, name) VALUES (1, 'orders')")
        .execute(&pool)
    ...
    sqlx::query("INSERT INTO branches (id, service_id, name) VALUES (1, 1, 'main')")
        ...
        .await
        .unwrap();
    sqlx::query("INSERT INTO endpoints (id, branch_id, path, method, yaml_content) VALUES (1, 1, '/orders/{id}', 'GET', 'openapi: 3.0.0')")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO clients (id, name) VALUES (1, 'checkout')")
        ...
        .await
        .unwrap();
    sqlx::query("INSERT INTO dependencies (id, client_id, endpoint_id, requested_service_id, requested_branch_name, requested_path, requested_method) VALUES (1, 1, 1, 1, 'main', '/orders/{id}', 'GET')")
        .execute(&pool)
        .await
        .unwrap();

    sqlx::query(include_str!(
        "../src/infrastructure/migrations/postgres/20240316000000_api_type.sql"
    ))
    .execute(&pool)
    .await
    .unwrap();

    let old_endpoint_constraints: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM pg_constraint con JOIN pg_class rel ON rel.oid = con.conrelid JOIN pg_namespace nsp ON nsp.oid = rel.relnamespace WHERE nsp.nspname = current_schema() AND rel.relname = 'endpoints' AND con.contype = 'u' AND ARRAY(SELECT att.attname FROM unnest(con.conkey) WITH ORDINALITY AS cols(attnum, ord) JOIN pg_attribute att ON att.attrelid = con.conrelid AND att.attnum = cols.attnum ORDER BY cols.ord) = ARRAY['branch_id', 'path', 'method']")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(old_endpoint_constraints, 0);

    sqlx::query("INSERT INTO endpoints (branch_id, api_type, path, method, yaml_content) VALUES (1, 'asyncapi', '/orders/{id}', 'GET', 'asyncapi: 3.0.0')")
        .execute(&pool)
        .await
        .unwrap();
